...
import 'package:modeling/lab7/bloc/smo_bloc.dart';

class SeventhLab extends StatelessWidget {
  const SeventhLab({Key? key}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Лабораторная работа №7"),
      ),
      body: SafeArea(
        child: Center(
          child: SizedBox(
                    ...
                    child: Column(
                      children: [
                        Wrap(
                          spacing: 30,
                          children: [
                            BlocBuilder<SmoBloc, SmoState>(
                              builder: (context, state) {
                                return ElevatedButton(
                                  onPressed: () {
                                    BlocProvider.of<SmoBloc>(context).add(
                                      const SmoEvent.run(),
                                    );
                                  },
